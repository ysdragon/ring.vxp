...
int read_from_file_to_addr(const char *path_, void **addr) {
	...
	if (!path_ || !addr) {
		log_write(
			"read_from_file_to_addr: Invalid arguments (path or addr is NULL)");
		if (addr)
			*addr = NULL;
		return 0;
	}
	*addr = NULL;
	...
	if (f < 0) {
		log_printf("File not found or cannot open: %s (Error code: %d)", path,
				   f);
		return 0;
	}
	...
	*addr = malloc(size + 1);
	if (*addr == NULL) {
		log_printf("Memory allocation failed for file content. Requested size: "
				   "%u bytes.",
				   size + 1);
		vm_file_close(f);
		return 0;
	...
	if (vm_file_read(f, *addr, size, &red) < 0 || red != size) {
		log_printf("File read error or incomplete read (read %u of %u bytes).",
				   red, size);
		free(*addr);
		*addr = NULL;
		vm_file_close(f);
