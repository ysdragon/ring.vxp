...
void handle_sysevt(VMINT message, VMINT param) {
	...
	case VM_MSG_CREATE:
		if (layer_hdl[0] == -1) {
			layer_hdl[0] =
				vm_graphic_create_layer(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, -1);
...
static void draw_running_state(void) {
	...
	int hint_text_y = SCREEN_HEIGHT - HINT_BAR_HEIGHT +
					  (HINT_BAR_HEIGHT - vm_graphic_get_character_height()) / 2;
	vm_graphic_textout_to_layer(
		layer_hdl[0], SCREEN_WIDTH - 5 - vm_graphic_get_string_width(ucs2_str),
		hint_text_y, ucs2_str, vm_wstrlen(ucs2_str));
...
static VMBOOL execute_script(const char *script_path) {
	...
	if (strlen(script_path) == 0) {
		...
		int text_y = (SCREEN_HEIGHT - HINT_BAR_HEIGHT -
					 ...
					 2;
		vm_graphic_textout_to_layer(layer_hdl[0], text_x, text_y, ucs2_str,
									vm_wstrlen(ucs2_str));
		vm_graphic_flush_layer(layer_hdl, 1);
		return VM_FALSE;
	}
	...
	do {
		int script_size =
			read_from_file_to_addr(script_path, &script_content_orig);

		if (!script_content_orig || script_size < 0) {
			log_printf(
				"Error loading script content (size: %d) or script is empty.",
				script_size);
	...
	log_write("Script execution successful (or finished). Ring state remains "
			  "active.");
	return VM_TRUE;
}