...
void draw_menu() {
	...
	vm_graphic_textout_to_layer(layer_hdl[0], title_x, title_y, ucs2_str,
								vm_wstrlen(ucs2_str));

	color.vm_color_565 = SEPARATOR_COLOR;
	vm_graphic_setcolor(&color);
	vm_graphic_line_ex(layer_hdl[0], 0, TITLE_HEIGHT, SCREEN_WIDTH - 1,
					   TITLE_HEIGHT);

	int menu_area_y = TITLE_HEIGHT + 1;
	int menu_area_height =
		SCREEN_HEIGHT - TITLE_HEIGHT - 1 - SOFTKEY_HEIGHT - 1;
	...
	for (int i = 0; i < items_on_screen; i++) {
		...
		vm_graphic_textout_to_layer(layer_hdl[0], 10, text_y, ucs2_str,
									vm_wstrlen(ucs2_str));
	}
	...
	if (total_items > items_on_screen) {
		...
		if (first_item_index > 0) {
			...
			vm_graphic_textout_to_layer(layer_hdl[0], SCREEN_WIDTH - 12,
										menu_area_y + 2, ucs2_str, 1);
		}
		if (first_item_index + items_on_screen < total_items) {
			vm_ascii_to_ucs2(ucs2_str, 128, "v");
			vm_graphic_textout_to_layer(layer_hdl[0], SCREEN_WIDTH - 12,
										menu_area_y + menu_area_height -
	...
	color.vm_color_565 = SEPARATOR_COLOR;
	vm_graphic_setcolor(&color);
	vm_graphic_line_ex(layer_hdl[0], 0, softkey_separator_y, SCREEN_WIDTH - 1,
					   softkey_separator_y);
	...
	int softkey_text_y =
		softkey_y + (SOFTKEY_HEIGHT - vm_graphic_get_character_height()) / 2;

	if (menu != MENU_ABOUT) {
		vm_graphic_textout_to_layer(layer_hdl[0], 5, softkey_text_y,
									select_text, vm_wstrlen(select_text));
	}

	if (menu != MENU_MAIN) {
		vm_graphic_textout_to_layer(
			layer_hdl[0],
			SCREEN_WIDTH - 5 - vm_graphic_get_string_width(back_text),
	...
	case MENU_FILE_PICKER: {
		...
		if (find_handle >= 0) {
			do {
				...
				if (attr >= 0 && !(attr & VM_FS_ATTR_DIR)) {
					char ascii_name[FILENAME_MAX];
					vm_ucs2_to_ascii(ascii_name, FILENAME_MAX,
									 fileinfo.filename);
...
void menu_confirm() {
	if (menu_list == NULL || menu_choice < 0 ||
		menu_choice >= arrlen(menu_list)) {
		return;
	}
	...
	case MENU_FILE_PICKER:
		...
		if (strncmp(selected_item, "[DIR] ", 6) == 0) {
			...
			strncat(currentDir, dir_name,
					FILENAME_MAX - strlen(currentDir) - 2);
			strcat(currentDir, "\\");
		...
		if (strncmp(selected_item, "[DIR]", 5) != 0 &&
			strcmp(selected_item, "Empty directory") != 0) {
	...
	case MENU_FILE_ACTION: {
		if (!strcmp(selected_item, "Run")) {
			...
			strncat(currentScript, file_action_path,
					FILENAME_MAX - strlen(currentScript) - 1);
...
void handle_keyevt_menu(VMINT event, VMINT keycode) {
	...
	if (keycode == VM_KEY_OK || keycode == VM_KEY_NUM5 ||
		keycode == VM_KEY_LEFT_SOFTKEY) {
		menu_confirm();
		return;
	}
	...
	if (menu_choice != prev_choice && layer_hdl[0] != -1 &&
...
void restore_menu_state() {
	if (last_menu_state.menu_type != MENU_MAIN ||
		last_menu_state.menu_choice != 0) {
